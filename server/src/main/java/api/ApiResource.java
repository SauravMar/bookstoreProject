...
public class ApiResource {
    ...
    public Category categoryByName(@PathParam("category-name") String categoryName,
        ...
        try {
            ...
            throw new ApiException("category name lookup failed", e);
        }
    }
    ...
    public List<Book> booksByCategoryName(@PathParam("category-name") String categoryName,
                                   @Context HttpServletRequest request) {
        try {
            Category catObject = categoryDao.findByName(categoryName);
            return bookDao.findByCategoryId(catObject.getCategoryId());
        } catch (Exception e) {
            throw new ApiException("category name lookup failed", e);
        }
    }
    ...
    public List<Book> suggestedBooksByCategoryName(@PathParam("category-name") String categoryName,
                                                   ...
                                                   @Context HttpServletRequest request) {
        try {
            return bookDao.findRandomByCategoryId(
                    categoryDao.findByName(categoryName).getCategoryId(),
                    limit
            );
        } catch (Exception e) {
            throw new ApiException("category name lookup failed", e);
